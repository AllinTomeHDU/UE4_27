...
enum mysql_option {
  MYSQL_OPT_CONNECT_TIMEOUT,		// 设置超时连接，传入超时阈值
  MYSQL_OPT_COMPRESS,				// 压缩
  MYSQL_OPT_NAMED_PIPE,				// 命名管道
  MYSQL_INIT_COMMAND,				// 传入字符串，在连接初始阶段自动执行的命令
  MYSQL_READ_DEFAULT_FILE,			// 读取默认选项文件，而不是在 my.cnf 里读
  MYSQL_READ_DEFAULT_GROUP,			// 默认组，从 my.cnf 读取选项
  MYSQL_SET_CHARSET_DIR,			// 指向含有字符集定义的目录
  MYSQL_SET_CHARSET_NAME,			// 默认字符集名称
  MYSQL_OPT_LOCAL_INFILE,			// 运行 LOAD DATA INFILE 此类命令
  MYSQL_OPT_PROTOCOL,				// MySQL协议，在 mysql_protocol_type 结构里面
  MYSQL_SHARED_MEMORY_BASE_NAME,	// 命名与服务器通信共享内存对象
  MYSQL_OPT_READ_TIMEOUT,			// 读取超时
  MYSQL_OPT_WRITE_TIMEOUT,			// 写入超时，只在 Windows TCP 连接上有效
  MYSQL_OPT_USE_RESULT,				// 
  MYSQL_REPORT_DATA_TRUNCATION,		// 是否允许预处理语句，默认禁止
  MYSQL_OPT_RECONNECT,				// 显性方式设置再连接行为，传入bool是否启用断线重连
  MYSQL_PLUGIN_DIR,					// 插件路径
  MYSQL_DEFAULT_AUTH,				// 默认最权威（服务器）
  MYSQL_OPT_BIND,					// 若客户端主机有多个网络接口，绑定IP/Port
  MYSQL_OPT_SSL_KEY,				//
  MYSQL_OPT_SSL_CERT,				//
  MYSQL_OPT_SSL_CA,					//
  ...
  MYSQL_OPT_SSL_CRL,				//
  MYSQL_OPT_SSL_CRLPATH,			// SSL 加密
  MYSQL_OPT_CONNECT_ATTR_RESET,		//
  MYSQL_OPT_CONNECT_ATTR_ADD,		//
  MYSQL_OPT_CONNECT_ATTR_DELETE,	// 链接属性设置
  MYSQL_SERVER_PUBLIC_KEY,			// RSA 公钥
  MYSQL_ENABLE_CLEARTEXT_PLUGIN,	// 是否启用客户端明文插入认证
  MYSQL_OPT_CAN_HANDLE_EXPIRED_PASSWORDS,  // 是否需要处理过期密码
  MYSQL_OPT_MAX_ALLOWED_PACKET,		//
  MYSQL_OPT_NET_BUFFER_LENGTH,		// 通信缓冲区
  MYSQL_OPT_TLS_VERSION,			// 是否允许加密连接的协议
  MYSQL_OPT_SSL_MODE,				// 服务器连接安全状态
